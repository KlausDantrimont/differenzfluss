# Der Brillenladen

## Eine epistemische Zwischensprache für KI-Systeme

**Klaus Dantrimont · 2026 · Version 2**

Ich arbeite viel mit KI-Systemen.

Dabei ist mir immer wieder etwas Merkwürdiges passiert:
Eine Antwort war klug, plausibel und gut formuliert – und trotzdem hatte ich das Gefühl, dass sie am Problem vorbeiging.

Nicht unbedingt, weil etwas falsch war.
Oft schien vielmehr die **Art des Hinschauens** nicht zu passen.

Die KI suchte beispielsweise nach Ursachen, obwohl mich eigentlich ein Prozess interessierte.
Sie betrachtete einzelne Akteure, obwohl vielleicht Strukturen entscheidend waren.
Oder sie erklärte eine Situation schlüssig, ohne sichtbar zu machen, welche andere Perspektive zu einer ganz anderen Erklärung geführt hätte.

Ich hatte dafür zunächst keine Lösung.

Aber mit der Zeit entstand ein kleines Arbeitsformat, das sich als nützlich erwies:

Perspektiven lassen sich teilweise in relativ einfache Denkbewegungen zerlegen, explizit machen und neu kombinieren.

Ich nenne solche Kombinationen **Brillen**.

Der folgende Text beschreibt diesen Versuch.

Er behauptet nicht, den richtigen Blick auf die Welt gefunden zu haben.

Er untersucht etwas Bescheideneres:

> **Wie können wir sichtbar machen, mit welcher Brille gerade gedacht wird – und wie können wir sie wechseln?**

Künstliche Intelligenz kann erstaunlich gute Antworten geben.
Sie kann analysieren, vergleichen, erklären, Hypothesen bilden, Szenarien entwickeln und Widersprüche finden.

Trotzdem bleibt bei vielen dieser Leistungen etwas merkwürdig unsichtbar:

**Aus welcher Perspektive wurde eigentlich gedacht?**

Eine KI kann dasselbe Problem psychologisch betrachten, technisch, historisch, institutionell, kausal, normativ oder als Rückkopplungssystem.

Sie kann nach Akteuren fragen oder nach Strukturen, nach Zuständen oder Prozessen, nach Evidenz oder nach Bedeutung.

Meist entscheidet sie darüber implizit.

Der Nutzer sieht die Antwort.

Die Wahl des Blicks bleibt im Hintergrund.

Vielleicht liegt hier eine bisher unterschätzte Schnittstelle.

Nicht zwischen Mensch und Maschine.

Sondern zwischen **Gegenstand und Denken**.

---

## Vor der Antwort liegt die Frage

Jede Analyse beginnt mit einer Auswahl.

Was betrachten wir?
Welche Unterschiede sind relevant?
Welche Beziehungen interessieren?
Welche zeitliche Skala wählen wir?
Suchen wir nach Ursachen?
Nach Funktionen?
Nach Interessen?
Nach Zuständen?
Nach Übergängen?
Nach Regeln?
Nach Macht?
Nach Evidenz?

Der Gegenstand selbst beantwortet diese Fragen nicht.

Sie bestimmen vielmehr, **welcher Gegenstand uns überhaupt erscheint**.

Eine Schule führt eine neue Regel ein.
Die Zahl der gemeldeten Konflikte steigt.

Man kann fragen:

Welche Wirkung hat die Regel?
Wie verändert sich das Verhalten über die Zeit?
Was wird überhaupt als Konflikt gemeldet?
Welche institutionellen Strukturen entstehen?
Wie erleben Schüler, Lehrkräfte und Eltern dieselbe Entwicklung?

Jede dieser Fragen schneidet anders durch dieselbe Situation.

Keine muss falsch sein.

Aber keine ist die Situation selbst.

---

# Vor der Frage liegt der Auftrag

Hier beginnt eine zweite Ebene.

Warum wollen wir etwas eigentlich wissen?

Diese Frage wirkt zunächst beinahe trivial.
Bei genauerem Hinsehen verändert sie jedoch die gesamte Architektur.

Nicht jede Untersuchung dient demselben Zweck.

Manchmal wollen wir wissen, **was wahr ist**.

Manchmal müssen wir eine Entscheidung treffen.

Manchmal wollen wir ein Problem beheben.

Manchmal möchten wir verstehen, warum sich jemand auf eine bestimmte Weise verhält.

Manchmal reicht eine grobe Orientierung.

Manchmal suchen wir gezielt nach einer Widerlegung unserer bisherigen Annahmen.

Und manchmal wollen wir etwas lieber **nicht** wissen.

Damit erscheint vor der Perspektivwahl eine bisher leicht übersehene Größe:

> **der epistemische Auftrag**

Der Auftrag beschreibt, **wozu Erkenntnis in einer konkreten Situation dienen soll**.

Er ist nicht identisch mit Wahrheit.

Auch ein System, das Wahrheit ernst nimmt, kann unterschiedliche Erkenntnisziele verfolgen.

Ein Arzt benötigt andere Informationen als ein Historiker.
Eine Ingenieurin muss vielleicht keine vollständige Theorie eines Fehlers entwickeln, wenn sie ihn zuverlässig beseitigen kann.
Ein Gericht stellt andere Fragen als ein Therapeut.
Ein Wissenschaftler benötigt andere Evidenz als jemand, der entscheiden muss, ob er morgen einen Regenschirm mitnimmt.

Die Frage

> **Welche Brille liefert den größten Erkenntnisgewinn?**

ist deshalb unvollständig.

Vollständiger wäre:

> **Welche Brille liefert für den gegenwärtigen Auftrag den relevantesten Erkenntnisgewinn?**

Damit erhält der Brillenladen eine zusätzliche Ebene.

Nicht mehr nur:

**Gegenstand → Perspektive → Erkenntnis**

sondern:

**Auftrag → Gegenstand → Perspektive → Erkenntnis**

oder praktisch:

**Irritation → Auftragsklärung → Brillenkonstruktion → Analyse**

---

## Ein neuer Operator: ZIEL

Diese Beobachtung legt einen weiteren elementaren Operator nahe.

**ZIEL**

Welche Zustände sollen erreicht, erhalten oder vermieden werden?

Der Operator fragt beispielsweise:

Welche expliziten Ziele bestehen?
Welche impliziten Ziele lassen sich begründet vermuten?
Welche Ziele konkurrieren miteinander?
Welche Ziele sind Mittel für andere Ziele?
Welche Ziele werden nur behauptet?
Welche Messgrößen dienen lediglich als Stellvertreter?
Was darf für ein Ziel geopfert werden?
Welchen Stellenwert besitzt Wahrheit innerhalb dieser Zielstruktur?

ZIEL ist nicht dasselbe wie ANREIZ.

ANREIZ fragt:

> **Welche Konsequenzen machen ein Verhalten wahrscheinlicher oder unwahrscheinlicher?**

ZIEL fragt:

> **Worauf soll das Verhalten hinauslaufen?**

Auch Ziele dürfen nicht vorschnell psychologisiert werden.

Ein beobachteter Effekt beweist keine Absicht.

Deshalb gilt als Leitplanke:

> **Ziele werden rekonstruiert, nicht unterstellt. Wo ihre Zuschreibung unsicher ist, bleiben sie Hypothesen.**

---

## Der Vorteil der Explizitifizierung

Viele der Operationen des Brillenladens sind für sich genommen nicht neu.

Menschen wechseln Perspektiven.
Experten erkennen Muster.
Analysen werden beendet, wenn weitere Arbeit kaum noch etwas bringt.
Texte bevorzugen bestimmte Arten von Erklärung.
Menschen verfolgen Ziele.
KI-Systeme tun vieles davon ebenfalls.

Der entscheidende Schritt liegt woanders:

> **Etwas, das bisher implizit geschieht, wird so explizit gemacht, dass man damit arbeiten kann.**

Man könnte diesen Vorgang etwas unfeierlich **Explizitifizierung** nennen.

Was explizit ist, kann benannt, beschrieben, getrennt, kombiniert, verglichen, kritisiert, protokolliert und getestet werden.

Es kann zwischen Systemen übertragen und technisch gesteuert werden.

Der Brillenladen erfindet Perspektivwahl nicht.

Er macht sie explizit.

Er erfindet auch nicht den Perspektivwechsel, die Blindstelle, das Aufhören, die Gegenprobe oder das Ziel.

Er versucht, solche ohnehin stattfindenden Denkbewegungen **adressierbar** zu machen.

> **Das Triviale bekommt eine Schnittstelle.**

Damit verändert sich auch die Frage an KI-Systeme.

Nicht nur:

**Kann die KI verschiedene Perspektiven einnehmen?**

Sondern:

**Kann sie sichtbar machen, welche sie gewählt hat?**
**Kann sie begründen, warum?**
**Kann sie angeben, welchem Auftrag diese Perspektive dient?**
**Kann sie erkennen, wann die Perspektive nicht mehr trägt?**
**Kann sie eine andere wählen?**
**Kann sie aufhören?**

Gerade darin liegt möglicherweise der praktische Wert der Explizitifizierung.

---

# Epistemische Operatoren

Der Brillenladen beginnt mit einer einfachen Idee:

Vielleicht lassen sich viele Perspektivbewegungen auf relativ elementare epistemische Operationen zurückführen.

Zum Beispiel:

**ZEIT**
Wie verändert sich etwas?

**ZUSTAND**
Welche Konfiguration liegt vor?

**RELATION**
Was steht womit in Beziehung?

**KAUSALITÄT**
Was bewirkt was?

**PERSPEKTIVE**
Wie verändert sich das Bild mit dem Beobachter?

**INFORMATION**
Wer weiß wann was, und über welche Kanäle?

**EVIDENZ**
Wodurch wird eine Aussage getragen?

**INSTITUTION**
Welche Strukturen reproduzieren sich dauerhaft?

**ANREIZ**
Welche Konsequenzen verändern Verhalten?

**SELEKTION**
Welche Varianten bleiben bestehen, welche verschwinden?

**SKALA**
Was verändert sich, wenn wir die Betrachtungsebene wechseln?

**RÜCKKOPPLUNG**
Welche Wirkungen wirken auf ihre eigenen Ursachen zurück?

**ZIEL**
Welche Zustände sollen erreicht, erhalten oder vermieden werden?

Solche Begriffe sind keine Theorie darüber, **was die Welt ist**.

Sie sind Operationen darüber, **wie man sie betrachten kann**.

Das ist ein wichtiger Unterschied.

Der Brillenladen behauptet nicht, die Wirklichkeit bestehe aus Zeit, Relation, Institution, Selektion, Perspektive oder Ziel.

Er sagt nur:

Diese Schnitte können Erkenntnis erzeugen.

---

# Brillen sind Kombinationen

Ein einzelner Operator ist noch keine vollständige Perspektive.

Interessant wird die Sache durch Kombination.

Für ein intermittierendes Problem in einem Softwaresystem könnten beispielsweise vier Operatoren besonders ergiebig sein:

**ZEIT · ZUSTAND · RELATION · INFORMATION**

Die zeitliche Dimension macht sichtbar, dass die Störung nicht dauerhaft besteht.

Der Zustandsoperator lenkt Aufmerksamkeit darauf, dass ein Neustart offenbar etwas zurücksetzt.

RELATION verhindert, nur einzelne Komponenten isoliert zu betrachten.

INFORMATION fragt, ob die vorhandenen Metriken überhaupt jene Zustände erfassen, die für die Störung relevant sind.

Aus vier relativ einfachen Schnitten entsteht eine spezifische Analyseperspektive.

Eine andere Szene benötigt eine andere Kombination.

Das ist der eigentliche Brillenladen:

Nicht ein Regal voller unveränderlicher Weltanschauungen.

Sondern ein **Baukasten für Perspektiven**.

---

# So wenig wie möglich

Dabei gilt ausdrücklich nicht:

> Je mehr Perspektiven, desto besser.

Jede zusätzliche Perspektive kostet Aufmerksamkeit und Komplexität.

Wer zwanzig Operatoren gleichzeitig auf ein kleines Problem richtet, bekommt möglicherweise keine tiefere Analyse, sondern nur mehr Text.

Eine gute Brille soll deshalb möglichst klein sein.

Das Prinzip lautet:

> **So wenig epistemische Struktur wie möglich, so viel wie für gute Orientierung nötig.**

Dazu gehört ein epistemisches Budget.

Für jeden zusätzlichen Operator lässt sich fragen:

Welchen neuen Schnitt bringt er?
Ist dieser Schnitt tatsächlich orthogonal zu den bereits verwendeten?
Oder beschreibt er fast dasselbe noch einmal?
Welche Blindstelle schließt er?
Wie groß ist der erwartete Erkenntnisgewinn?
Was kostet die zusätzliche Komplexität?

Mit dem Begriff des Auftrags kommt eine weitere Frage hinzu:

> **Ist die zusätzliche Erkenntnis für den gegenwärtigen Auftrag überhaupt relevant?**

Ein möglicher Erkenntnisgewinn kann real und trotzdem unnötig sein.

Damit verändert sich das epistemische Budget.

Nicht einfach:

**Erkenntnisgewinn / Aufwand**

sondern eher:

**auftragsrelevanter Erkenntnisgewinn / epistemische und praktische Kosten**

Und irgendwann:

**Reicht es?**

Auch das Beenden einer Analyse ist eine epistemische Operation.

---

# Wahrheit und Auftrag

Hier ist eine Trennung wichtig.

Der Auftrag bestimmt, **welche Erkenntnis gesucht wird**.

Er bestimmt nicht, **was wahr ist**.

Ein technisches Team kann beschließen, nur so tief zu analysieren, wie es zur Fehlerbehebung nötig ist.

Das macht falsche Aussagen nicht wahr.

Eine politische Entscheidung kann unter Zeitdruck getroffen werden.

Das ändert die Evidenzlage nicht.

Ein Mensch kann entscheiden, eine Frage momentan nicht weiterzuverfolgen.

Das verändert nicht die Realität, auf die sich die Frage bezieht.

Der Auftrag steuert also den Erkenntnisprozess.

Er darf nicht mit einem Wahrheitskriterium verwechselt werden.

Gerade deshalb sollte eine epistemische Zwischensprache beides getrennt halten:

> **Wozu wollen wir etwas wissen?**

und:

> **Was dürfen wir aufgrund der verfügbaren Evidenz für wahr halten?**

---

# Epistemische Bereitschaft

Damit tritt ein weiterer Gegenstand in den Blick.

Menschen wollen nicht immer alles wissen, was sie wissen könnten.

Das ist leicht zu übersehen, wenn man Wahrheitssuche selbst als selbstverständlichen Auftrag betrachtet.

Doch Wissen kann Kosten erzeugen.

Eine Information kann verlangen, dass man seine Meinung verändert.

Eine Beziehung neu bewertet.

Eine Entscheidung trifft.

Eine Verpflichtung anerkennt.

Ein Selbstbild korrigiert.

Eine Gruppe enttäuscht.

Oder etwas nicht mehr ignorieren kann.

Dann konkurriert Erkenntnis mit anderen Zielen:

Zugehörigkeit.
Sicherheit.
Handlungsfähigkeit.
Loyalität.
Selbstachtung.
Ruhe.
Status.
Hoffnung.

Dadurch entsteht eine weitere zusammengesetzte Perspektive:

## EPISTEMISCHE BEREITSCHAFT

Sie lässt sich beispielsweise aus folgenden Operatoren konstruieren:

**ZIEL · INFORMATION · PERSPEKTIVE · ANREIZ · NORM · ROLLE**

Ihre Leitfrage lautet:

> **Welche Erkenntnisse kann oder will ein Akteur an sich heranlassen, prüfen und in sein Handeln integrieren – und wodurch wird diese Bereitschaft begrenzt?**

Dabei lassen sich mindestens drei Ebenen unterscheiden.

### Aufnahme

Was gelangt überhaupt in den relevanten Wahrnehmungs- und Aufmerksamkeitsraum?

Was erscheint interessant?

Was wird übersehen?

Was wirkt plötzlich irrelevant, unglaubwürdig oder nebensächlich?

### Prüfung

Welche Fragen werden ernsthaft weiterverfolgt?

Welche Hypothesen werden getestet?

Welche Gegenargumente erhalten Aufmerksamkeit?

Wo wird die Suche beendet?

### Integration

Welche gewonnenen Erkenntnisse dürfen tatsächlich Folgen haben?

Welche dürfen das Selbstbild verändern?

Welche dürfen Beziehungen verändern?

Welche dürfen Verhalten oder Entscheidungen verändern?

Diese drei Ebenen sind nicht identisch.

Ein Mensch kann eine Information wahrnehmen und sogar intellektuell akzeptieren, ohne sie praktisch zu integrieren.

---

# Welchen Auftrag erfüllt die Verformung?

Damit verändert sich die Analyse epistemischer Verzerrung.

Man kann eine verzerrte Wahrnehmung selbstverständlich weiterhin als epistemisches Problem beschreiben.

Aber daraus folgt nicht automatisch, dass sie innerhalb des gesamten psychischen oder sozialen Systems eine Fehlfunktion darstellt.

Wenn der primäre Auftrag lautet:

**Finde heraus, was wahr ist, auch wenn es unangenehm wird,**

dann ist selektive Wahrnehmung eine Störung dieses Auftrags.

Wenn der dominierende Auftrag dagegen lautet:

**Bewahre Zugehörigkeit,**

**halte ein handlungsfähiges Selbstbild aufrecht,**

**vermeide einen existenziell wahrgenommenen Konflikt**

oder

**erfülle deine Rolle innerhalb einer Gemeinschaft,**

kann dieselbe epistemische Verzerrung funktional sein.

Dann lautet eine analytisch stärkere Frage nicht sofort:

> **Warum verbiegt dieser Mensch die Wirklichkeit?**

Sondern zunächst:

> **Welchen Auftrag erfüllt diese Verformung möglicherweise?**

Oder genauer:

> **Welche Ziele, Bindungen oder Verpflichtungen würden durch eine unverzerrtere Wahrnehmung gefährdet?**

Das entschuldigt die Verzerrung nicht.

Es erklärt sie auch nicht automatisch.

Aber es verhindert, epistemische Abweichung vorschnell mit innerer Korruption gleichzusetzen.

---

# Epistemische und normative Integrität

Damit lassen sich drei Formen von Kohärenz unterscheiden.

**Epistemische Integrität**

Treue zu Evidenz, Korrekturbereitschaft und möglichst wahrheitsgemäßer Modellbildung.

**Normative Integrität**

Treue zu den eigenen Werten, Verpflichtungen und moralischen Maßstäben.

**Systemische Kohärenz**

Das Zusammenspiel der verschiedenen Teile eines Systems ohne größere innere Widersprüche.

Diese drei Größen können zusammenfallen.

Sie müssen es nicht.

Ein Mensch kann epistemisch sorgfältig sein und gegen seine eigenen Werte handeln.

Er kann normativ überzeugt handeln und dabei Tatsachen systematisch verzerren.

Und er kann innerhalb eines geschlossenen Weltbildes außerordentlich kohärent sein, obwohl wesentliche Annahmen falsch sind.

Das ist besonders wichtig für KI-Systeme, die menschliche Argumentation analysieren sollen.

Innere Kohärenz ist kein Wahrheitsbeweis.

Und epistemische Verzerrung ist nicht automatisch ein Beweis für moralische Verderbtheit.

---

# Dynamische Brillenkonstruktion

Damit kann eine KI mehr tun, als eine vorgegebene Perspektive anzuwenden.

Sie kann die Perspektive selbst konstruieren.

Gegeben sei eine Szene, ein Problem oder eine Irritation.

Die KI fragt zunächst:

**Wozu soll die Untersuchung dienen?**

Dann:

**Was ist hier eigentlich erklärungsbedürftig?**

Anschließend wählt sie aus dem Operatorenkatalog eine möglichst kleine Menge von Schnitten mit hohem erwarteten Erkenntnisgewinn **relativ zum Auftrag**.

Sie begründet ihre Auswahl.

Sie benennt auch naheliegende Operatoren, die sie zunächst bewusst nicht benutzt.

Dann führt sie die Perspektive aus.

Anschließend untersucht sie ihr eigenes Restproblem:

Was wurde gut erklärt?
Was bleibt offen?
Welche Blindstellen erzeugt die gewählte Brille selbst?
Ist ein weiterer Schnitt nötig?
Hat sich der Auftrag während der Analyse verändert?
Oder ist zusätzliche Analyse ohne neue Information nur noch dekorative Komplexität?

Damit bekommt Perspektivwahl einen kontrollierbaren Ablauf.

Die KI analysiert nicht nur einen Gegenstand.

Sie analysiert auch **ihre eigene Analysearchitektur**.

---

# Auftragsklärung als Metaoperation

Damit entsteht eine Metaoperation, die vor der eigentlichen Brillenwahl liegt.

## AUFTRAGSKLÄRUNG

Ihre Leitfrage lautet:

> **Wozu wird diese Untersuchung durchgeführt?**

Mögliche Antworten sind beispielsweise:

Orientierung.
Wahrheitsprüfung.
Entscheidung.
Fehlerbehebung.
Prognose.
Verstehen.
Konfliktklärung.
Ideengenerierung.
Risikominimierung.
Selbstprüfung.

Die Auftragsklärung kann auch mehrere Ziele sichtbar machen.

Dann muss nicht sofort eines ausgewählt werden.

Man kann zunächst feststellen:

> **Hier konkurrieren verschiedene epistemische Aufträge.**

Zum Beispiel:

Eine Organisation möchte wissen, warum ein Projekt scheitert.

Gleichzeitig möchte sie Verantwortliche nicht beschädigen.

Ein Mensch möchte wissen, ob eine Beziehung noch trägt.

Gleichzeitig möchte er die Antwort möglicherweise nicht kennen.

Ein politischer Akteur möchte eine Lage realistisch einschätzen.

Gleichzeitig muss er eine öffentliche Erzählung stabil halten.

Gerade solche Zielkonflikte sind epistemisch relevant.

---

# Die inverse Operation

Wenn man aus Operatoren eine Perspektive konstruieren kann, lässt sich die Bewegung auch umkehren.

Gegeben sei nun keine Szene, sondern ein Bericht.

Eine Erzählung.
Eine politische Rede.
Ein wissenschaftlicher Text.
Ein Strategiepapier.
Eine persönliche Darstellung eines Konflikts.

Dann lautet die Frage nicht:

**Welche Brille sollten wir verwenden?**

Sondern:

> **Welche Brille wird hier bereits verwendet?**

Welche epistemischen Schnitte strukturieren die Erzählung?
Welche Arten von Zusammenhang gelten als erklärungsrelevant?
Was wird sichtbar?
Was tritt in den Hintergrund?

Ein Text könnte beispielsweise hauptsächlich durch

**KAUSALITÄT · ANREIZ · ROLLE**

strukturiert sein.

Dann erscheinen Menschen vor allem als handelnde Akteure, Entscheidungen als Ursachen und Verhalten als Reaktion auf Konsequenzen.

Ein anderer Bericht über dasselbe Geschehen könnte überwiegend mit

**ZEIT · INSTITUTION · RÜCKKOPPLUNG**

arbeiten.

Plötzlich erscheinen einzelne Akteure weniger wichtig.

Langfristige Strukturen und selbststabilisierende Prozesse treten hervor.

Beide Berichte können denselben Gegenstand behandeln.

Und doch konstruieren sie epistemisch verschiedene Welten.

Mit ZIEL kommt nun eine zusätzliche Frage hinzu:

> **Auf welchen impliziten oder expliziten Auftrag scheint diese Darstellung zugeschnitten zu sein?**

Damit wird nicht nur die Brille rekonstruierbar.

Auch ihre mögliche Funktion tritt in den Blick.

---

# Epistemische Faktorisierung

Diese Rückwärtsbewegung lässt sich als **epistemische Faktorisierung** verstehen.

Man versucht, eine komplexe Perspektive auf eine möglichst kleine Menge tragender Operatoren zurückzuführen.

Nicht:

Welche Operatoren könnte man irgendwie im Text finden?

Sondern:

> **Welche minimale Kombination erklärt den charakteristischen Blick dieser Darstellung?**

Dabei muss die Faktorisierung nicht eindeutig sein.

Ein Text kann mehrere Perspektiven mischen.

Operatoren können sich teilweise überschneiden.

Manche Zuordnung bleibt unsicher.

Gerade deshalb braucht die Rekonstruktion Prüfungen.

Was geschieht, wenn man einen vermuteten Operator entfernt?

Verliert der Text dadurch einen wesentlichen Teil seiner Struktur?

Könnte ein anderer Operator dieselben Bewegungen besser erklären?

Sind zwei ausgewählte Operatoren wirklich verschieden oder nur unterschiedliche Namen für denselben Schnitt?

Das Ergebnis ist kein psychologisches Profil des Autors.

Es sagt nicht:

**So denkt dieser Mensch.**

Es sagt:

**So wird in diesem Text gedacht.**

Und möglicherweise:

**Diesem erkennbaren oder vermuteten Auftrag dient die Darstellung.**

Auch diese zweite Aussage muss selbstverständlich als Hypothese markiert werden, sofern der Auftrag nicht explizit genannt wird.

---

# Sichtbarkeit und Blindstellen

Jede Brille erzeugt Sichtbarkeit.

Und jede Brille erzeugt Blindstellen.

Das ist kein Vorwurf.

Ein Bericht kann unmöglich alles gleichzeitig darstellen.

Interessant wird jedoch die Frage:

Welche für den Gegenstand naheliegenden Operatoren fehlen?

Ein Konflikt wird vielleicht vollständig über individuelle Absichten erklärt.

Dann fehlen möglicherweise institutionelle oder historische Schnitte.

Ein ökonomischer Bericht betrachtet nur Anreize und Kennzahlen.

Vielleicht fehlen Macht, Bedeutung oder langfristige Rückkopplungen.

Ein moralischer Bericht arbeitet vor allem mit NORM und ROLLE.

Vielleicht bleibt die Frage nach Evidenz schwach.

Mit dem Begriff epistemischer Bereitschaft kommt eine weitere Möglichkeit hinzu:

Vielleicht fehlt ein Schnitt nicht zufällig.

Vielleicht wäre seine Anwendung für den gegenwärtigen Auftrag störend.

Auch daraus darf nicht vorschnell Manipulation abgeleitet werden.

Die neutralere Feststellung lautet weiterhin:

> **Diese Darstellung erzeugt diese Sichtbarkeit und diese Blindstellen.**

Danach kann man fragen:

Sind die Blindstellen methodisch notwendig?
Entstehen sie aus begrenzter Information?
Aus dem Auftrag?
Aus Anreizen?
Aus Normen?
Aus Rollen?
Aus fehlender epistemischer Bereitschaft?
Oder aus strategischer Auswahl?

Erst danach sollte über Bias, Manipulation oder Täuschung gesprochen werden.

---

# Keine Wahrheitsmaschine

Der Brillenladen entscheidet nicht, was wahr ist.

Das ist eine wichtige Grenze.

Ein hervorragend konstruierter kausaler Blick kann auf falschen Daten beruhen.

Eine perfekt faktorisierte Erzählung kann unwahre Behauptungen enthalten.

Eine multiperspektivische Analyse kann sich trotzdem irren.

Und ein klar formulierter Auftrag kann epistemisch fragwürdig sein.

Epistemische Struktur und Wahrheit sind nicht dasselbe.

Der Brillenladen arbeitet eine Ebene davor.

Er macht sichtbar,

**welche Fragen gestellt werden,**
**welchem Auftrag sie dienen,**
**welche Schnitte verwendet werden,**
**welche Arten von Erklärung entstehen**
und
**welche Alternativen zunächst unsichtbar bleiben.**

Danach können Evidenzprüfung, Recherche, Experiment, Statistik oder Argumentation einsetzen.

Der Brillenladen ersetzt diese Verfahren nicht.

Er kann helfen zu entscheiden, **welche davon überhaupt gebraucht werden**.

---

# Eine Zwischensprache

Damit lässt sich das Konzept genauer einordnen.

Der Brillenladen ist kein Prompt-Katalog.

Keine Sammlung von Expertenrollen.

Kein Multi-Agent-System.

Keine Ontologie.

Keine vollständige Erkenntnistheorie.

Er ist eher eine:

> **epistemische Zwischensprache für KI-Systeme.**

Sein Vokabular besteht aus elementaren epistemischen Operatoren.

Seine Grammatik besteht aus deren Kombination.

Seine Metaebene beginnt mit der Auftragsklärung.

Seine Optimierungsregel ist das epistemische Budget.

Seine Vorwärtsoperation ist die Konstruktion einer Perspektive.

Seine inverse Operation ist deren Faktorisierung.

Sein Zweck ist Orientierung.

Man könnte es auch technischer formulieren:

> **Ein kompositioneller Operatorenkatalog zur Konstruktion und Rekonstruktion von Analyseperspektiven unter expliziten Erkenntniszielen.**

Die Formulierung klingt größer als ein Brillenladen.

Gemeint ist dasselbe.

---

# Zwischen Benutzer und KI

Eine unmittelbare Anwendung liegt in normalen KI-Sitzungen.

Heute gibt man einer KI häufig Rollen:

> Sei ein kritischer Analyst.
> Denke wie ein Historiker.
> Betrachte das als Psychologe.

Solche Anweisungen funktionieren erstaunlich gut.

Sie sind aber epistemisch unscharf.

Was genau macht einen „kritischen Analysten“ aus?

Welche Denkbewegungen soll er bevorzugen?

Welche vermeiden?

Mit Operatoren lässt sich eine Sitzung expliziter konfigurieren.

Beispielsweise:

> **Auftrag:** Finde die wahrscheinlichste technische Ursache.
> Betrachte das Problem zunächst mit
> ZEIT + RELATION + GEGENHYPOTHESE.
> Verwende KAUSALITÄT erst, wenn genügend Evidenz vorliegt.

Oder:

> **Auftrag:** Verstehe die Darstellungsstruktur dieses Berichts.
> Rekonstruiere zuerst seine dominante Perspektive.
> Führe anschließend genau einen unterrepräsentierten Operator als Gegenprobe ein.

Oder:

> **Auftrag:** Untersuche, warum eine bestimmte Information offenbar nicht integriert wird.
> Verwende ZIEL + INFORMATION + ANREIZ + NORM.
> Unterstelle keine bewusste Täuschung.

Damit bekommt die KI nicht bloß eine Rolle.

Sie bekommt eine **epistemische Arbeitsweise**.

---

# Zwischen Anwendung und Reasoning-System

Die gleiche Idee lässt sich tiefer in KI-Systeme einbauen.

Man kann sich eine Architektur vorstellen:

**Benutzer oder Anwendung**

↓

Problem
Bericht
Entscheidung
Forschungsfrage

↓

**Auftragsklärung**

↓

**epistemische Zwischenschicht**

Operatoren
Brillenkonstruktion
Faktorisierung
Blindstellenanalyse
epistemisches Budget
epistemische Bereitschaft

↓

**Reasoning-System**

Analyse
Recherche
Agenten
Hypothesenbildung
Simulation
Synthese

Die Operatorenschicht entscheidet dabei nicht selbst über Inhalte.

Sie strukturiert den Erkenntnisprozess.

Das macht sie potenziell unabhängig vom konkreten Sprachmodell.

Unterschiedliche Modelle können dieselbe Spezifikation erhalten.

Sie werden vermutlich unterschiedlich damit arbeiten.

Gerade das ist interessant.

Die Operatoren definieren nicht das Ergebnis.

Sie definieren den **Raum erlaubter oder bevorzugter Denkbewegungen**.

---

# Mehrere Agenten, mehrere Brillen

Auch Multi-Agent-Systeme lassen sich damit strukturieren.

Heute werden Agenten oft als Figuren konstruiert:

der Optimist,
der Skeptiker,
der Pragmatiker,
der Experte.

Das ist anschaulich.

Man könnte Agenten jedoch auch epistemisch definieren.

Agent A untersucht:

**ZEIT · PROZESS · RÜCKKOPPLUNG**

Agent B:

**ANREIZ · ROLLE · MACHT**

Agent C:

**EVIDENZ · GEGENHYPOTHESE · KAUSALITÄT**

Agent D:

**ZIEL · INFORMATION · NORM · PERSPEKTIVE**

Dann unterscheiden sich die Agenten nicht primär durch Persönlichkeit.

Sie unterscheiden sich durch **Analyseoperationen**.

Das könnte Konflikte zwischen Agenten präziser machen.

Man weiß nicht nur, dass sie verschiedener Meinung sind.

Man kann rekonstruieren, **warum sie unterschiedliche Dinge sehen**.

Zusätzlich kann man prüfen, ob sie überhaupt denselben Auftrag verfolgen.

Vielleicht entsteht der Konflikt nicht aus unterschiedlichen Antworten.

Vielleicht beantworten die Agenten verschiedene Fragen.

---

# Forschung und Analyse

Für Forschungsprozesse liegt eine weitere Anwendung nahe.

Eine Forschungsfrage kann vorab epistemisch profiliert werden:

Welchen Auftrag verfolgt die Untersuchung?
Welche Operatoren verwenden wir bereits?
Welche fehlen?
Sind mehrere angeblich konkurrierende Theorien vielleicht gar keine direkten Konkurrenten, weil sie unterschiedliche Schnitte verwenden?
Ist ein Streit kausal, normativ, begrifflich oder perspektivisch?

Auch Literaturübersichten könnten faktorisierend gelesen werden.

Nicht nur:

Wer behauptet was?

Sondern:

> **Welche epistemischen Operationen dominieren ein Forschungsfeld?**

Vielleicht wird ein Gegenstand jahrzehntelang fast ausschließlich kausal betrachtet.

Oder ausschließlich auf individueller Ebene.

Ein Skalenwechsel oder institutioneller Schnitt könnte dann nicht bloß eine neue Antwort liefern.

Er könnte eine neue **Frageklasse** eröffnen.

Noch interessanter wird die Frage:

> **Welche Fragen werden in einem Forschungsfeld kaum gestellt, weil sie nicht zu seinen institutionellen, methodischen oder normativen Aufträgen passen?**

Damit verbindet sich Perspektivanalyse mit Wissenschaftssoziologie, ohne auf diese reduziert zu werden.

---

# Journalismus, Politik und öffentliche Erzählungen

Auch öffentliche Texte lassen sich auf diese Weise untersuchen.

Eine politische Erzählung kann stark normativ und akteursbezogen konstruiert sein.

Eine andere institutionell und historisch.

Eine dritte ökonomisch und kausal.

Statt sofort zu fragen:

**Wer hat recht?**

kann man zunächst fragen:

**Welche Art von Welt erzeugt diese Darstellung?**

Und nun zusätzlich:

**Welchem Auftrag dient diese Darstellung möglicherweise?**

Informieren?
Mobilisieren?
Beruhigen?
Legitimieren?
Warnen?
Identität stabilisieren?
Verantwortung zuschreiben?

Diese Fragen beantworten die Wahrheitsfrage nicht.

Aber sie können sichtbar machen, warum bestimmte Informationen betont und andere kaum wahrgenommen werden.

Das ist besonders nützlich, wenn zwei Seiten scheinbar über dasselbe sprechen, tatsächlich aber auf unterschiedlichen epistemischen Ebenen und unter verschiedenen Aufträgen operieren.

Manchmal widersprechen sich ihre Antworten.

Manchmal stellen sie schlicht verschiedene Fragen.

Manchmal wollen sie sogar verschiedene Dinge **wissen**.

Diese Unterscheidung kann Konflikte nicht lösen.

Aber sie kann sie genauer lokalisieren.

---

# Bildung

Für Bildung ließe sich der Operatorenkatalog stark reduzieren.

Schüler müssten keine epistemische Algebra lernen.

Ein kleiner Satz grundlegender Fragen könnte genügen:

Was verändert sich?
Was hängt womit zusammen?
Aus wessen Sicht?
Woher wissen wir das?
Welche Regeln wirken?
Was könnte noch eine Erklärung sein?
Was wollen wir mit dieser Untersuchung eigentlich herausfinden?

Damit würde der Brillenladen zu einer Grammatik des Fragens.

Nicht:

**Welche Antwort soll ich glauben?**

Sondern:

**Welche Frage fehlt noch?**

Und möglicherweise:

**Will ich die Antwort tatsächlich wissen?**

Gerade im Umgang mit KI könnte das wertvoll sein.

Denn eine Maschine, die jederzeit flüssige Antworten erzeugt, erhöht den Wert guter Fragen.

Und vielleicht auch den Wert einer Fähigkeit, die noch davor liegt:

**zu bemerken, welche Fragen man vermeidet.**

---

# Grenzen des Ansatzes

Der Brillenladen selbst braucht Kritik.

Es ist nicht selbstverständlich, dass sein Operatorenkatalog vollständig ist.

Vielleicht fehlen Operatoren.

Vielleicht sind manche redundant.

Vielleicht müssen andere aufgeteilt werden.

INFORMATION und EVIDENZ liegen beispielsweise nahe beieinander, stellen aber unterschiedliche Fragen.

Ebenso NORM und INSTITUTION.

Auch ZIEL muss sich bewähren.

Vielleicht ist es tatsächlich elementar.

Vielleicht zeigt sich später, dass verschiedene Formen von Zielgerichtetheit getrennt werden müssen.

Auch die epistemische Bereitschaft ist zunächst keine elementare Operation, sondern eine zusammengesetzte Brille.

Solche Grenzen werden oft erst im Gebrauch sichtbar.

Das ist kein unangenehmer Nebeneffekt.

Es ist Teil des Projekts.

Wenn Operatoren wirklich elementar sein sollen, müssen sie sich an sehr unterschiedlichen Gegenständen bewähren.

Eine gute Zerlegung sollte weder beliebig fein noch unnötig grob sein.

Die ideale Faktorisierung ist vermutlich nicht endgültig erreichbar.

Aber man kann sie verbessern.

---

# Modelle werden unterschiedlich wählen

Auch verschiedene KI-Systeme werden dieselbe Szene unterschiedlich faktorisieren.

Das ist zu erwarten.

Ein Modell hält vielleicht SELEKTION für zentral.

Ein anderes KAUSALITÄT.

Ein drittes beginnt mit SKALA.

Ein viertes erkennt zunächst einen Zielkonflikt.

Das muss kein Fehler des Verfahrens sein.

Die Brillenkonstruktion enthält selbst Urteil.

Entscheidend ist deshalb nicht, dass jedes Modell dieselben Operatoren wählt.

Interessanter ist:

Kann es seine Auswahl begründen?
Kann es Redundanz erkennen?
Kann es Blindstellen benennen?
Kann es den Auftrag explizit machen?
Kann es bemerken, dass mehrere Aufträge konkurrieren?
Kann es nach einem ersten Schnitt feststellen, dass ein weiterer nötig ist?
Kann es auch aufhören?

Gerade diese Unterschiede zwischen Modellen könnten selbst Gegenstand einer epistemischen Analyse werden.

---

# Brillen bauen und Brillen erkennen

Damit entsteht eine bemerkenswerte Symmetrie.

Vorwärts:

**Operatoren → Perspektive**

Rückwärts:

**Perspektive → Operatoren**

Mit der neuen Ebene lässt sich die Bewegung erweitern.

Vorwärts:

**Auftrag → Irritation → Operatorenauswahl → Brille → Analyse**

Rückwärts:

**Erzählung → Darstellungsstruktur → Operatoren → epistemisches Profil → möglicher Auftrag**

Die erste Bewegung erzeugt einen Blick.

Die zweite rekonstruiert einen vorhandenen.

Zusammen bilden sie eine kleine Sprache für Perspektiven.

Nicht für die Welt selbst.

Sondern für die Art, wie wir sie schneiden.

---

# Wozu das alles?

Vielleicht besteht ein Teil intelligenter Orientierung darin, nicht nur Antworten erzeugen zu können.

Sondern bemerken zu können:

**Welche Frage stelle ich gerade?**
**Warum diese?**
**Wozu möchte ich die Antwort wissen?**
**Welche andere Frage wäre möglich?**
**Was macht meine Perspektive sichtbar?**
**Was kann sie prinzipiell schlecht sehen?**
**Was bin ich bereit zu erfahren?**
**Was würde sich ändern, wenn ich die Antwort ernst nähme?**

Menschen tun das gelegentlich.

KI-Systeme können dabei helfen.

Aber dafür brauchen sie eine Sprache, in der Perspektiven explizit behandelt werden können.

Der Brillenladen ist der Versuch, eine solche Sprache bereitzustellen.

Klein genug, um praktisch benutzt zu werden.

Abstrakt genug, um in verschiedenen Domänen zu funktionieren.

Kompositionell genug, um neue Perspektiven zu erzeugen.

Und offen genug, um sich selbst verändern zu lassen.

Vielleicht muss eine intelligente Maschine nicht immer dieselbe Brille tragen.

Vielleicht muss sie auch nicht möglichst viele gleichzeitig tragen.

Vielleicht genügt zunächst die Fähigkeit,

**zu wissen, dass sie eine trägt,**
**zu wissen, welchem Auftrag sie dient,**
**sie beschreiben zu können,**
**eine passendere zu bauen**

und

**sie wieder abzusetzen, wenn sie ihren Zweck erfüllt hat.**

Das wäre schon einiges.
