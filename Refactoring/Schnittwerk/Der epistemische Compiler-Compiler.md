...
Schnittwerk beschreibt epistemische Arbeit bislang vor allem auf der Ebene von:
...
Dies ist noch kein Prompt.

Es ist auch noch kein Programm für ein bestimmtes LLM.
...
Der Compiler-Compiler übersetzt diese Methodenspezifikation in eine ausführbare Working-Set-Strategie.
...
Der Generator beantwortet damit die Frage:
...
Damit ergeben sich zwei grundsätzlich verschiedene Compilationsvorgänge:
...
R1 liefert dafür einen allgemeinen Maßstab.
...
Damit braucht nicht jede epistemische Methode eine völlig eigene Definition von Kontextqualität.

Die Methode spezifiziert:

> **Welche relevante Leistung muss erhalten bleiben?**
...
Damit wird R1 zu einer Art **generischer Semantik guter Working Sets**.
...
Die vorhandenen R2-Operationen lassen sich dabei unmittelbar verwenden:
...
Auf Kontext angewandt bedeutet beispielsweise:

### REMOVE

Kann dieses Objekt entfernt werden, ohne die relevante epistemische Leistung zu beeinträchtigen?

### SPLIT

Enthält ein Dokument mehrere epistemisch unterschiedliche Informationsarten, von denen nur eine benötigt wird?

### MERGE

Sind mehrere Informationsobjekte funktional redundant?

### EXTRACT

Kann aus umfangreicher Historie eine kleine relevante Struktur herausgezogen werden?

### REFRAME

Ist nicht der Kontext zu schlecht, sondern bereits der epistemische Auftrag falsch geschnitten?

Damit ist R2 nicht bloß eine Inspirationsquelle.

Es kann zur **generischen Transformationslogik des Compilers** werden.
...
Ein vollständig statisches System kann bereits:
...
R3 erweitert dieses System später um Lernen.

Dann könnte beispielsweise gelernt werden:
...
Damit würde die Signatur eine ähnliche Rolle übernehmen wie eine IR in klassischen Compilersystemen:

> verschiedene Eingabesprachen können auf eine gemeinsame Zwischenrepräsentation abgebildet werden.
...
Eine epistemische Methode könnte auf unterschiedliche Substrate übersetzt werden:
...
Damit bleibt ein Grundgedanke von R2 erhalten:
...
Der Begriff Compiler darf nicht zu eng verstanden werden.

Ein erzeugter Working-Set Compiler könnte beispielsweise sein:
...
Ein sehr einfacher Compiler könnte selbst wieder nur eine Spezifikation sein:
...
Die entscheidende Eigenschaft ist nicht seine Implementationssprache.

Entscheidend ist:

> **Er transformiert einen epistemischen Zustand nach einer expliziten Methode in einen ausführbaren Working Set.**

---

# 15. Determinismus und KI

Der Compiler-Compiler muss nicht alles einem LLM überlassen.
...
> **Explizite Regeln dort, wo Regeln möglich sind.  
> Semantisches Urteil dort, wo Bedeutung beurteilt werden muss.**
...
Dann entsteht kein improvisierter Sachoutput.

Stattdessen erzeugt er beispielsweise:
...
Der Compiler kann nach Beschaffung der Information erneut laufen.
...
Schnittwerk wäre nicht nur:

> eine Architektur zur Beschreibung epistemischer Problemräume.

Es könnte zugleich die Grundlage einer **deklarativen Sprache für epistemische Programme** bilden.

Ein epistemisches Programm spezifiziert nicht vollständig die einzelnen Gedankenschritte.

Es spezifiziert vielmehr:
...
In einer größeren Architektur könnte die Rollenverteilung so aussehen:
...
Der Compiler-Compiler verbindet damit die **Methodenschicht** mit der **Laufzeitschicht**.

---

# 20. Minimal Viable Compiler-Compiler

Für einen ersten Proof of Concept ist keine komplexe Infrastruktur nötig.
...
Erst danach muss untersucht werden:
...
Ein einfacher Test könnte vier Bedingungen vergleichen:

```text
A
gesamter verfügbarer Kontext

B
Similarity Retrieval

C
manuell entworfener Working-Set Compiler

D
automatisch generierter Working-Set Compiler
```
...
Der Compiler-Compiler verändert die bisherige Interpretation des Schnittwerks.
